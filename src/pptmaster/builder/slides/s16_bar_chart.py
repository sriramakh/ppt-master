"""Slide 16 — Bar Chart: Full-width clustered bar chart."""
...
def build(slide, *, theme=None) -> None:
    ...
    content_top = add_slide_title(slide, c.get("bar_title", "Revenue by Region"), theme=t)
    ...
    spec = ChartSpec(
        ...
        categories=c.get("bar_categories", ["NA", "EU", "APAC", "LATAM", "MEA", "Africa"]),
        series=c.get("bar_series", [{"name": "FY 2025", "values": [320, 210, 180, 95, 65, 45]}]),
...
def _scholarly(slide, t):
    """White bg, 'Figure 2:' caption, thin rule frame around chart."""
    c = t.content
    m = MARGIN
    title = c.get("bar_title", "Revenue by Region")

    add_background(slide, "#FFFFFF")
    content_y = add_slide_title(slide, title, theme=t)

    # Figure caption above chart
    caption = f"Figure 2: {title}"
